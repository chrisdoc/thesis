...
\If {$Intersects(curr,next)$}
   \State $centroid=Centroid(Intersects(curr,next))$
   \State $Add(HP,NearestPoint(route,centroid))$
\Else
	\State $line=LineBetweenNearestPoint(curr,next)$
    \If {$Intersects(line,route)$}
        \State $Add(HP,Intersects(line,route))$
        \Else
        \State $midPoint=MidPoint(line)$
        \State $Add(HP,NearestPoint(route,midPoint))$
    \EndIf
\EndIf
\end{algorithmic}
