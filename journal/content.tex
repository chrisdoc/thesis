...
The first validation calculates the squared sum of the minimum distance between the route and the centroid of each cell areas as can be seen in (\ref{eq:sumsquaremine}). This is the same metric that Tettamanti et al.~\cite{Tettamanti2012} used for their system. For each of the previously calculated routes $j$, the squared sum of all minimum distances $d_{i,j}$ between the route and the cell site was calculated.
...
%
% \State $HP\gets List()$
% \For{$i \leftarrow 1,length(handover)-1$}
...
%\EndIf
%\end{algorithmic}
%\caption{Handover position prediction}\label{alg:prediction}
%\end{figure}
\begin{figure}
\centering
...
%\begin{figure}
%\begin{algorithmic}[1]
% \For{$i \leftarrow 1,length(trajectory)$}
% \State $allowedSpeed=MaxSpeed(trajectory[i])$
% \State $c=trajectory[i]$
%  \State $p=trajectory[i-1]$
%   \State $n=trajectory[i+1]$
%  
%\If {$Speed(c)\ge1.7*allowedSpeed$}
%   \State $cDist=Distance(c)$
%    \State $pDist=Distance(p)$
%     \State $nDist=Distance(n)$
%   \State $nomDist=allowedSpeed*Duration(c)$
%   \State $SetSpeed(c,allowedSpeed);$
%    \If {$p==NULL\&\&n!=NULL$}
%        \State $pDist=pDist+(cDist-nomDist)$
%        \State $SetSpeed(p,pDist/Duration(p))$
%        \ElsIf {$p!=NULL\&\&n==NULL$}
%         \State $nDist=nDist+(cDist-nomDist)$
%         \State $SetSpeed(n,nDist/Duration(n))$
%        \Else
%        \State $nTempDist=nDist+(cDist+nomDist)/2$
%        \State $pTempDist=pDist+(cDist+nomDist)/2$
%        \If {$nTempDist/Duration(n)\gg allowedSpeed$}
%        \EndIf
%    \EndIf
%\EndIf
%\EndFor
%\end{algorithmic}
%\caption{Handover adaption}\label{alg:adaption}
%\end{figure}
...
In Fig.~\ref{fig:563} and \ref{fig:144} the first and the fourth trajectories are illustrated. Each of the three coverage prediction methods -- Network planning tool (a), Network planning tool with buildings (b) and Voronoi diagrams (c) -- along with the estimated handover position is depicted. The last Fig.\ (d) shows the route of the test drive with the exact location of each handover. Due to the limited place the second and third trajectory was omitted.
